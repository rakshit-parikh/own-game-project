...
document.getElementById(id).disabled = true;
 ...
 { document.getElementById("splayer").innerHTML = ply.color + "'s Turn"; } function MultishowTurn(ply)
 ...
 { alert("you win");
  reset();
   setTimeout(enableButton("dicebtn"),500);
    setTimeout(enableButton("reset"),500); } else setTimeout(bot,1000); 
 ...
 function menable() { 
     ...
     // setup function for single player 
     { showGame();
     senable();
      getcolor();
    ...
    players[i].roll(random_number);
     ...
     players[k].show(players[k].position);
      ...
      if(globalpostion <= players[i].position) globalpostion = players[i].position;
       if(globalpostion == 100) { alert(newFunction() + " Wins"); reset();
     }
      else{ ++i; if(i == nop) i = 0;
     MultishowTurn(players[i]);
     } function newFunction() { return players[i].color;
     } }
      function playmulti() { 
    rand();
     myfun(); 
    ...
    menable();
     ...
     for(var k = 0; k < i; ++k) { if(color == players[k].color) { alert("Not available");
     color = prompt("Enter color :"); break;
     } }
